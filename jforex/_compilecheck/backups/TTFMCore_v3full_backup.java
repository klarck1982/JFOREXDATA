...
public class TTFMCore implements IIndicator, IDrawingIndicator {
    ...
    /** [O/approx] T-Spot zone = overlap of (respected half of last completed HTF candle) with (trend-shift candle wick range). */
    static double[] tspotZone(CandleData lastCompleted,CandleData trendShift,int bias){
        if (lastCompleted==null||trendShift==null||bias==0) return null;
        double eq=lastCompleted.eq();
        double aLo=bias>0?eq:lastCompleted.low;
        double aHi=bias>0?lastCompleted.high:eq;
        double lo=Math.max(aLo,trendShift.low), hi=Math.min(aHi,trendShift.high);
    ...
    public void onStart(IIndicatorContext context){
        ...
        opt.add(new OptInputParameterInfo("[Struct] Show OB / Breaker",OptInputParameterInfo.Type.OTHER,new IntegerListDescription(1,BOOLEAN_VALUES,BOOLEAN_NAMES)));
        set.add(v->showOb=((Integer)v)==1);
        opt.add(new OptInputParameterInfo("[TSpot] Show T-Spot (approx)",OptInputParameterInfo.Type.OTHER,new IntegerListDescription(1,BOOLEAN_VALUES,BOOLEAN_NAMES)));
    ...
    private void updateConceptState(RB[] bars,LayerData primary,long periodMs){
        ...
        CandleData lastC=null,shiftC=null;
        for (int i=hist.size()-1;i>=0;i--) if (hist.get(i).completed){
            if (lastC==null) lastC=hist.get(i);
            else { shiftC=hist.get(i); break; }
        }
        tspotZoneD=tspotZone(lastC,shiftC,currentBias);
    }
    ...
    private void drawTspot(Graphics2D g2,IIndicatorDrawingSupport support){
        if (tspotZoneD==null) return;
        int yT=(int)support.getYForValue(tspotZoneD[1]), yB=(int)support.getYForValue(tspotZoneD[0]);
        if (yB<=yT) return;
        int x=support.getChartWidth()-220;
        g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,0.22f));
        g2.setColor(new Color(255,215,0));
        g2.fillRect(x,yT,200,yB-yT);
        g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,1.0f));
        g2.setColor(new Color(255,215,0));
        g2.setFont(g2.getFont().deriveFont(Font.BOLD,9f));
        g2.drawString("T-Spot (approx)",x+4,yT-3);
    }
