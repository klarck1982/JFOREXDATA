...
public class TTFMCore implements IIndicator, IDrawingIndicator {
    ...
    private boolean showClosures = false;  // step-build: off until agreed
    private int closureColorIndex = 0;
    private int closureStyleIndex = 1;
    private int closureWidth = 1;
    private int shadeMode = 0;          // step-build: off until agreed
    ...
    private boolean showCISD = false;   // step-build: off until agreed
    ...
    // v2
    private boolean colorStates = false; // step-build: off until agreed
    private boolean skipLateIC = false;
    private boolean showBias = false;   // step-build: off until agreed
    private boolean showTargets = false; // step-build: off until agreed
    // v3/v4/v5
    private boolean showFvg = false;    // step-build: off until agreed
    private boolean showOb = false;     // step-build: off until agreed
    private boolean showTspot = true;
    private boolean showInfoPanel = false; // step-build: off until agreed
    ...
    static boolean c2Closure(CandleData c1,CandleData c2,boolean bullish,boolean poiInHalf,int structTrend){
        ...
        if (!(c2.close>=c1.low&&c2.close<=c1.high)) return false;
    ...
    static double[] tspotZone(CandleData cN,CandleData cN1){
        ...
        return tspotZone(cN.candleEQ(),cN1.open);
    }
    /** [C] zone type: 1 = Discount (below EQ, demand/green), 0 = Premium (above EQ, supply/red). */
    ...
    public void onStart(IIndicatorContext context){
        ...
        opt.add(new OptInputParameterInfo("[Display] Show Closures",OptInputParameterInfo.Type.OTHER,new IntegerListDescription(0,BOOLEAN_VALUES,BOOLEAN_NAMES)));
        ...
        opt.add(new OptInputParameterInfo("[CISD] Detection",OptInputParameterInfo.Type.OTHER,new IntegerListDescription(0,BOOLEAN_VALUES,BOOLEAN_NAMES)));
        ...
        opt.add(new OptInputParameterInfo("[State] Color States (gray/orange/red)",OptInputParameterInfo.Type.OTHER,new IntegerListDescription(0,BOOLEAN_VALUES,BOOLEAN_NAMES)));
        set.add(v->colorStates=((Integer)v)==1);
        opt.add(new OptInputParameterInfo("[Proj] Show Targets",OptInputParameterInfo.Type.OTHER,new IntegerListDescription(0,BOOLEAN_VALUES,BOOLEAN_NAMES)));
        set.add(v->showTargets=((Integer)v)==1);
        opt.add(new OptInputParameterInfo("[Struct] Show FVG Zones",OptInputParameterInfo.Type.OTHER,new IntegerListDescription(0,BOOLEAN_VALUES,BOOLEAN_NAMES)));
        ...
        opt.add(new OptInputParameterInfo("[SMT] Detection (Pivot-based)",OptInputParameterInfo.Type.OTHER,new IntegerListDescription(0,BOOLEAN_VALUES,BOOLEAN_NAMES)));
        set.add(v->smtEnabled=((Integer)v)==1);
        opt.add(new OptInputParameterInfo("[Info] Show Concept Panel",OptInputParameterInfo.Type.OTHER,new IntegerListDescription(0,BOOLEAN_VALUES,BOOLEAN_NAMES)));
    ...
    public java.awt.Point drawOutput(Graphics g,int outputIdx,Object values,Color color,
        ...
        for (int i=0;i<MAX_LAYERS;i++){
            ...
            if (shadeMode==1&&i==firstLayerIndex){
                ...
                for (int k=disp.size()-1;k>=1&&(disp.size()-1-k)<total;k--){
                    ...
                    double prevMid=prev.candleEQ(), half=(prev.high-prev.low)/2.0;
        ...
        if (outputIdx==0){
            ...
            if (showFvg) drawFvgZones(g2,support);
            if (showOb) drawObZones(g2,support);
            if (showTspot) drawTspot(g2,support);
    ...
    private void updateConceptState(RB[] bars,LayerData primary,long periodMs){
        ...
        for (int k=hist.size()-1;k>=1;k--){
            ...
            if (c2Closure(c1,c2,bull,poiInRespectedHalf(c1,bull),structTrend)){
                ...
                if (k+1<hist.size()&&c3Closure(c2,hist.get(k+1),bull)) c3Idx=k+1;
                break;
            }
        }
        ...
        for (int k=1;k<hist.size();k++){
            ...
            addTspotZone(cN.candleEQ(),cN1.open,cN1.openTime,cN1.openTime+pMs,bars,false);
        }
        if (primary.curActive && hist.size()>=1){
            CandleData cN=hist.get(hist.size()-1);
            if (cN.completed) addTspotZone(cN.candleEQ(),primary.curO,primary.curStart,primary.curStart+pMs,bars,true);
        }
    ...
    private void fetchSmt(long periodMs){
        ...
        try {
            ...
            for (String other:SMT_INSTRUMENTS){
                ...
                if (ob==null) continue;
                if (smtDivergence(mine,ob,true,50)){ smtLabel="SMT bull vs "+other; return; }
                if (smtDivergence(mine,ob,false,50)){ smtLabel="SMT bear vs "+other; return; }
            }
    ...
    private void drawFvgZones(Graphics2D g2,IIndicatorDrawingSupport support){
        for (double[] z:fvgZones){
            int x=support.getXForTime((long)z[3],false);
            if (x<0) continue;
            int yT=(int)support.getYForValue(z[1]), yB=(int)support.getYForValue(z[0]);
            if (yB<=yT) continue;
            int w=Math.max(8,support.getChartWidth()-x);
            g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,0.12f));
            g2.setColor(z[2]==1?new Color(0,180,0):new Color(220,0,0));
            g2.fillRect(x,yT,Math.min(w,300),yB-yT);
            g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,1.0f));
        }
    }
    ...
    private void drawTspot(Graphics2D g2,IIndicatorDrawingSupport support){
        for (double[] z:tspotZones){
            double lo=z[0],hi=z[1]; int type=(int)z[4],state=(int)z[5]; boolean current=z[6]==1;
            ...
            Color base=(type==1)?new Color(0,180,0):new Color(220,0,0);      // Discount green / Premium red
            Color fill=(state==2)?new Color(140,140,140):base;               // invalidated -> gray
            float alpha=(state==2)?0.08f:(state==1)?0.15f:0.30f;             // fresh > retested > invalidated
            g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,alpha));
            g2.setColor(fill);
            g2.fillRect(x1,yT,x2-x1,yB-yT);
            g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,1.0f));
            g2.setColor(fill);
            g2.setStroke(new BasicStroke(current?2.0f:1.0f,BasicStroke.CAP_BUTT,BasicStroke.JOIN_MITER,10f,(state==1)?new float[]{4f,3f}:null,0f));
            g2.drawRect(x1,yT,x2-x1,yB-yT);
            g2.setFont(g2.getFont().deriveFont(Font.BOLD,8f));
            String lbl=(type==1?"T-Spot D":"T-Spot P")+(state==2?"  [invalid]":state==1?"  [retest]":"");
            g2.drawString(lbl,x1+2,yT-2);
        }
    }

    // ---- drawing: concept info panel ----
    private void drawInfoPanel(Graphics2D g2,IIndicatorDrawingSupport support,Font oldFont){
        List<String> lines=new ArrayList<>();
        String trend=(structTrend>0?"UP":structTrend<0?"DOWN":"-");
        String ev=(structEvent==1?"BOS":structEvent==2?"CHoCH":"-");
        lines.add("STRUCT: "+trend+" / "+ev);
        lines.add("REV bull "+Integer.bitCount(revBullMask)+"/5  bear "+Integer.bitCount(revBearMask)+"/5");
        if (c2Idx>=0) lines.add("C2 closure OK"+(c3Idx>=0?"  +C3":"")+(Double.isNaN(c2Entry)?"":"  wickMid entry"));
        lines.add("Profile: "+profileName+"  EntryTF: "+tfShort(entryPairInterval)+"  EQTF: "+tfShort(eqPairInterval));
        if (smtLabel!=null) lines.add(smtLabel);
        if (tspotZoneD!=null) lines.add(String.format(Locale.US,"T-Spot %.5f-%.5f",tspotZoneD[0],tspotZoneD[1]));

        g2.setFont(oldFont.deriveFont(Font.BOLD,9f));
        FontMetrics fm=g2.getFontMetrics();
        int w=0; for (String l:lines) w=Math.max(w,fm.stringWidth(l));
        int x=10, y=support.getChartHeight()-20-lines.size()*(fm.getHeight()+2);
        g2.setColor(new Color(20,20,30,210));
        g2.fillRect(x-5,y-fm.getAscent()-5,w+14,lines.size()*(fm.getHeight()+2)+10);
        for (int i=0;i<lines.size();i++){
            g2.setColor(new Color(230,230,230));
            g2.drawString(lines.get(i),x,y+i*(fm.getHeight()+2));
        }
    }
